...
businesscomponents.previewandreports.Vocabularycte = businesscomponents.previewandreports.Vocabularycte || {};

businesscomponents.previewandreports.Vocabularycte.Question = function(optHtml) {

    toot.ui.Component.call(this, $(optHtml !== undefined ? optHtml : businesscomponents.previewandreports.Vocabularycte.Question.HTML)[0]);
...
toot.inherit(businesscomponents.previewandreports.Vocabularycte.Question, toot.ui.Component);


//可尝试用List渲染
toot.extendClass(businesscomponents.previewandreports.Vocabularycte.Question, {
        ...
        if (this._answers.getAnswers() && this._questions && (this._questions.length == this._answers.getAnswers().length)) {
            ...
            for (var i = 0; i < this._questions.length; i++) {
                ...
                if (this._answers.getAnswers()[i].getIsRight()) {
                    table += this._trIsRight1Html(i + 1, this._questions[i].getChineseWord(), this._answers.getAnswers()[i].getAnswer(), this._questions[i].getEnglishWord());
                    //正确：rightIcon
                    rightCount++;
                    ...
                    if (this._answers.getAnswers()[i].getIsRight2()) {
                        table += this._trIsRight2Html(i + 1, this._questions[i].getChineseWord(), this._answers.getAnswers()[i].getAnswer(), this._questions[i].getEnglishWord());
                        //正确：rightIcon
                        rightCount++;
                    } else {
                        //答案未填写
                        if (this._answers.getAnswers()[i].getAnswer() == null || $.trim(this._answers.getAnswers()[i].getAnswer()) == '') {
                            table += this._trIsEmptyHtml(i + 1, this._questions[i].getChineseWord(), this._answers.getAnswers()[i].getAnswer(), this._questions[i].getEnglishWord());
                            wrongCount++;
                        } else {
                            //回答错误
                            table += this._trIsWrongHtml(i + 1, this._questions[i].getChineseWord(), this._answers.getAnswers()[i].getAnswer(), this._questions[i].getEnglishWord());
                            wrongCount++;
                        }
                    }
                }
...
businesscomponents.previewandreports.Vocabularycte.Question.HTML = '<div class="ReportNewQAbox ReportNewStyle1">' +
